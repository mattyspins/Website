...
import { weeklyRaffleApi, WeeklyRaffle, WeeklyRaffleDrawResult } from "@/lib/api/weeklyRaffle";
        ...
        <div className="bg-red-500/8 border border-red-500/25 rounded-xl p-5">
          ...
          <div className="space-y-2">
            {pendingDraws.map((p) => {
              ...
              return (
                <div key={p.id} className="bg-navy-900/40 border border-white/6 rounded-lg overflow-hidden">
                  <div className="flex items-center justify-between gap-2 px-3 py-2">
                    ...
                    <span className="flex items-center gap-2 shrink-0">
                      <button
                        onClick={() => setExpandedPendingId(isExpanded ? null : p.id)}
                        className="flex items-center gap-1.5 bg-white/6 hover:bg-white/10 text-gray-300 px-2.5 py-1.5 rounded-lg text-xs font-semibold transition-colors"
                      >
                  ...
                  {isExpanded && (
                    <div className="border-t border-white/6 px-3 py-3">
                      <WeeklyRaffleParticipants raffleId={p.id} />
                    </div>
                  )}
                </div>
              );
          ...
          <div className="lg:col-span-2 space-y-4">
              ...
              {raffle.status === "OPEN" && (
                <>
                  <div className="flex items-center gap-2 text-sm text-gray-400 mb-4">
                    ...
                    {`${eligibleCount} participant${eligibleCount === 1 ? "" : "s"}`}
                  </div>
                  <button
                    onClick={handleDraw}
                    disabled={drawing || !eligibleCount}
            ...
            {raffle.status === "OPEN" && (
              <div className="bg-navy-800/60 border border-white/6 rounded-xl p-5">
                ...
                <WeeklyRaffleParticipants raffleId={raffle.id} onCountChange={setEligibleCount} />
              </div>
            )}
          </div>
