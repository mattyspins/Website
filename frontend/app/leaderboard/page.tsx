 ...
 */
async function getInitialData(): Promise<{ race: ActiveRace | null; history: RaceHistoryEntry[] } | null> {
  try {
    const [raceRes, historyRes] = await Promise.all([
      fetch(`${API_URL}/api/wager-leaderboard/active`, { cache: "no-store" }),
      fetch(`${API_URL}/api/wager-leaderboard/history`, { cache: "no-store" }),
    ]);
...
export default async function Page() {
  ...
  return <LeaderboardContent initialData={initialData} />;
}
